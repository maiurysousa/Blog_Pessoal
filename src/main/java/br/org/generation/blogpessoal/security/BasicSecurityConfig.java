...
/* Classe BasicSecurityConfig é responsável por habilitar a segurança básica da aplicação e o login
 * na aplicação.
 * 
 * A herança da Classe WebSecurityConfigurerAdapter será para habilitar a segurança HTTP no Spring
 * fornecendo uma configuração padrão no método configure (HttpSecurity http)
 * 
...
public class BasicSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private UserDetailsService userDetailsService;
	 /*O bjeto da Interface UserDetailsService que será implementada na 
	 * Classe UserDetailsServiceImpl que fará o acesso ao nosso Banco de dados
	 * para recuperar os dados do usuário.*/

	@Override
	protected void configure(AuthenticationManagerBuilder auth) throws Exception { //protected: permite acesso às classes filhas, mas proíbe a qualquer outro acesso externo.
		/*@Override tem a função 
	...
	public PasswordEncoder passwordEncoder() {
		...
		/*A annotation @Beantransforma a instância retornada pelo
		 *  método como um objeto gerenciado pelo Spring, desta forma,
		 *   ele pode ser injetado em qualquerclasse
 			a qualquer momento que você precisar sem a necessidade de usar a 
	...
	protected void configure(HttpSecurity http) throws Exception {
		 ...
		 * 
		 * HttpMethod.OPTIONS -> Permite que o cliente (frontend), possa descobrir quais são as opções de 
